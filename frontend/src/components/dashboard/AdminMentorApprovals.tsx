...
const statusStyles: Record<string, string> = {
  draft: "bg-neutral-100 text-zinc-700",
...
export function AdminMentorApprovals() {
  ...
  return (
    <div className="space-y-8">
      ...
      {rejectId ? (
        <form onSubmit={submitReject} className="rounded-md border border-neutral-200 bg-brand-surface p-5 dark:border-zinc-700 ">
          <h3 className="font-bold text-brand-ink dark:text-white">Reason for return</h3>
          <p className="mt-1 text-sm text-neutral-500">The mentor will see this and can update their profile.</p>
          <textarea
            required
            rows={3}
            className="mt-3 w-full rounded-md border border-neutral-200 px-3 py-2 text-sm dark:border-neutral-500 dark:bg-zinc-800"
            value={rejectReason}
            onChange={(e) => setRejectReason(e.target.value)}
          />
          <div className="mt-3 flex gap-2">
            <button type="submit" disabled={busy} className="btn-primary-brand rounded-md px-4 py-2 text-sm font-semibold">
              Send feedback
            </button>
            <button type="button" onClick={() => setRejectId(null)} className="rounded-md border px-4 py-2 text-sm">
              Cancel
            </button>
          </div>
        </form>
      ...
      {rows === null ? (
        ...
        <>
          <section>
            ...
            {pending.length === 0 ? (
              <p className="mt-3 text-sm text-neutral-500">No profiles waiting for approval.</p>
            ) : (
...
function MentorReviewCard({
  ...
  return (
    <article className="rounded-md border border-neutral-200/90 bg-brand-surface p-5 shadow-none dark:border-zinc-700 ">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex gap-4">
          <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-md bg-neutral-100 ring-2 ring-brand-accent/20">
            {photo ? (
          ...
          <div>
            ...
            <p className="text-sm text-neutral-500">{row.headline}</p>
        ...
        <div className="flex flex-wrap gap-2">
          {row.approval_status === "pending" ? (
            <>
              <button type="button" disabled={busy} onClick={onApprove} className="inline-flex items-center gap-1 rounded-sm bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-emerald-700">
                <Check className="h-3.5 w-3.5" />
                Approve
              </button>
              <button type="button" disabled={busy} onClick={onReject} className="inline-flex items-center gap-1 rounded-sm border border-red-200 px-3 py-1.5 text-xs font-semibold text-red-700 hover:bg-red-50">
          ...
          {row.approval_status === "approved" ? (
            <button type="button" disabled={busy} onClick={onToggleFeatured} className="inline-flex items-center gap-1 rounded-sm border border-brand-accent/40 bg-brand-accent/10 px-3 py-1.5 text-xs font-semibold text-brand-accent">
      ...
      <p className="mt-4 line-clamp-4 text-sm text-neutral-500 dark:text-neutral-400">{row.bio}</p>
      {row.rejection_reason ? (
        <p className="mt-3 rounded-sm bg-red-50 px-3 py-2 text-xs text-red-800">Last feedback: {row.rejection_reason}</p>
      ) : null}
    </article>
  );
}
